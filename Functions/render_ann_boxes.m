...
if isempty(handles.data.anncalls)
    calls_in_page = [];
else
    calls_in_page = find(strcmp({handles.data.anncalls.Audiodata.Filename}',handles.data.audiodata.Filename) & ...
        ((handles.data.anncalls.Box(:,1) >= axis_xlim(1) & handles.data.anncalls.Box(:,1) < axis_xlim(2)  ) ...
