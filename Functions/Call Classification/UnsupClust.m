...
%                         ClusteringData(:,'xTime_Contour_Sl') = contourtimesl;
            ...
            %             ninflpt     = cellfun(@(x) get_infl_pts((diff(x,2)-mu)./sigma),ClusteringData.xFreq,'UniformOutput',false);
                    ...
                    xline(medianS,':g','LineWidth',3);
                    xlim([-1 1])
                    yticklabels(1:size(C,1))

                    % For some reason a comment doesn't always mean a
                    % comment???? So moving these up here so I can save
                    % them in case I ever want to revert the graphs
                    ...
                    title(sprintf('Silhouettes of Clusters - %d Clusters',size(C,1)),...
                        {sprintf('Mean1 (overall mean, red dashed) = %0.2f  Mean2 (mean by clust) = %0.2f',meanS, accummu),...
                        sprintf('Med (green dotted) = %0.2f  Max (no Sngtons) = %0.2f  Prop<=0 = %0.2f',...
